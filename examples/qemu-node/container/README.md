...
after the guest application answers both TCP and UDP through a private readiness
forward on port 18002, then refreshes that evidence continuously. Data-plane
traffic remains on port 18001. Repeated failure or stale evidence is degraded.
...
kept in the QEMU container's private mode-0700 `/run/graphx-qemu` tmpfs, owned
by the configured non-root runtime user, and is not shared with telemetry or
the packet observer. Bounded proof is retained as
