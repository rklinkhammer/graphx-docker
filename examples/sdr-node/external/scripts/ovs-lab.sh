...
compose=$profile_dir/compose.ovs.yaml
...
load_state() {
  ...
  source "$state"
  test -n "${GRAPHX_M5_EXTERNAL_OWNER:-}" && test -n "${GRAPHX_SDR_TLS_DIR:-}"
  case "$GRAPHX_SDR_TLS_DIR" in "$profile_dir"/.state/m5-tls) ;; *) return 2 ;; esac
  export GRAPHX_SDR_TLS_DIR
}
save_state() {
  install -d -m 0700 "$(dirname "$state")"
  GRAPHX_M5_EXTERNAL_OWNER=$(openssl rand -hex 16)
  GRAPHX_SDR_TLS_DIR=$profile_dir/.state/m5-tls
  printf 'GRAPHX_M5_EXTERNAL_OWNER=%q\nGRAPHX_SDR_TLS_DIR=%q\n' \
    "$GRAPHX_M5_EXTERNAL_OWNER" "$GRAPHX_SDR_TLS_DIR" >"$state.tmp.$$"
  chmod 0600 "$state.tmp.$$"; mv "$state.tmp.$$" "$state"
  export GRAPHX_SDR_TLS_DIR
}
owned_pid() {
  local pid command
  test -r "$profile_dir/.state/m5-sdr.pid" || return 1
  pid=$(cat "$profile_dir/.state/m5-sdr.pid")
...
stop_sdr() {
  ...
  rm -f "$profile_dir/.state/m5-sdr.pid"
}
cleanup_external() {
...
rollback_up() {
  ...
  if test "$cleanup_status" -eq 0 && test "$core_status" -eq 0 && \
    ...
    echo "Rollback retained M5 SDR state for ownership-safe recovery" >&2
  fi
  return "$original"
}
...
case ${1:-} in
  up)
    ...
    test ! -e "$state" || { echo "M5 SDR lab is already owned" >&2; exit 2; }
    save_state
