...
const edges = Object.fromEntries(['samples', 'transformed'].map(id => [id, {
  messages: 0, received: 0, wireBytes: 0, drops: 0, rate: 0,
  latencyUs: 0, lastSequence: 0, lastSeen: null, windowCount: 0, windowStart: Date.now(),
}]))
...
const server = createServer((request, response) => {
  ...
  if (url.pathname.startsWith('/api/control/') && request.method === 'POST') {
    ...
    if (action === 'fault') state.fault = !state.fault
    if (action === 'reset') {
      state = { paused: false, fault: false, updatedAt: new Date().toISOString() }
      recent.length = 0
      for (const edge of Object.values(edges)) Object.assign(edge, { messages: 0, received: 0, wireBytes: 0, drops: 0, rate: 0, latencyUs: 0, lastSequence: 0, windowCount: 0, windowStart: Date.now() })
    }
    state.updatedAt = new Date().toISOString()
    broadcast()
    return json(response, 200, { accepted: action, state })
  }
...
udp.on('message', data => {
  try {
    ...
    if (edge) {
      ...
      if (event.event === 'error') edge.drops += 1
    }
    broadcast()
