#!/usr/bin/env python3
"""Privileged Linux network observation capture/fault ownership lifecycle regression."""
...
def main() -> int:
    if len(sys.argv) != 2 or sys.platform != "linux" or os.geteuid() != 0:
        raise SystemExit("network observation live test requires: test_network_observability_live.py GRAPHX as Linux root")
    for command in ("ovs-vsctl", "ip", "tc", "dumpcap", "capinfos"):
        if shutil.which(command) is None:
            raise SystemExit(f"network observation live test requires {command}")
    graphx = Path(sys.argv[1]).resolve()
    bridge, namespace = "br-gxobslive", "gx-network-observation-live"
    capture_root = Path("/var/lib/graphx/captures/network-observation-live-test")
...
network:
  ...
  switches:
    - id: {bridge}
      kind: openvswitch
      ports:
        - {{id: a, interface: gxobsa0, peer: gxobsa1}}
        - {{id: b, interface: gxobsb0, peer: gxobsb1}}
        - {{id: capture, interface: gxobscap0, peer: gxobscap1}}
  ...
  routers:
    - id: m7-peer
      ...
      forwarding: false
      interfaces:
        - {{id: a, network: data, address: 10.77.0.2/24, device: gxobsa1, peer: gxobsa0, switch: {bridge}}}
        - {{id: b, network: data, address: 10.77.0.3/24, device: gxobsb1, peer: gxobsb0, switch: {bridge}}}
  attachments:
    - {{id: a-data, kind: namespace_veth, owner: m7-peer, network: data,
       address: 10.77.0.2/24, interface: gxobsa1, peer: gxobsa0, switch: {bridge}}}
    - {{id: b-data, kind: namespace_veth, owner: m7-peer, network: data,
       address: 10.77.0.3/24, interface: gxobsb1, peer: gxobsb0, switch: {bridge}}}
    - {{id: m7-span, kind: mirror, owner: {bridge}, interface: gxobscap0, switch: {bridge}}}
  captures:
  ...
  faults:
    - {{id: live-delay, attachment: a-data, delay_ms: 5, jitter_ms: 1,
       duration_seconds: 2}}
""", encoding="utf-8")
            state_file = state / "network-observation-live.yaml"
            ...
            if failed.returncode == 0 or state_file.exists() or run(
                ...
                raise AssertionError("network observation injected failure did not roll back")
            cleanup(state_file)
            ...
            if crashed.returncode != 99 or not state_file.exists() or not list(
                ...
                raise AssertionError("network observation crash did not leave recoverable capture identity: "
            ...
            if state_file.exists() or run("ovs-vsctl", "br-exists", bridge,
                ...
                raise AssertionError("network observation recovery left infrastructure or ledger residue")
            cleanup(state_file)
            ...
            if not session_match or not timer_match:
                raise AssertionError("network observation ledger lacks adversarial-test identities")
            ...
            for _ in range(50):
                if run("kill", "-0", timer_pid, check=False).returncode != 0:
                    break
                time.sleep(0.02)
            run("tc", "qdisc", "del", "dev", "gxobsa0", "root")
            ...
            for cycle in range(3):
                ...
                if active_session_match is None:
                    raise AssertionError("network observation ledger lacks active capture directory")
                ...
                if not all(marker in status for marker in
                    ...
                    raise AssertionError(f"network observation status lacks active capture/fault evidence: {status}")
                if not all(marker in ledger for marker in
                    ...
                    raise AssertionError("network observation ledger lacks process/qdisc identities")
                if "netem" not in run("tc", "qdisc", "show", "dev", "gxobsa0").stdout:
                    raise AssertionError("declarative netem was not applied")
                frame_sender = ("import socket; s=socket.socket(socket.AF_PACKET, "
                                "socket.SOCK_RAW); s.bind(('gxobsa1', 0)); "
                                "s.send(bytes.fromhex('ffffffffffff02000000007788b5') + "
                                "b'graphx-network-observation' * 8)")
                for _ in range(6):
                ...
                if "netem" in run("tc", "qdisc", "show", "dev", "gxobsa0").stdout:
                ...
                if state_file.exists() or run("ovs-vsctl", "br-exists", bridge,
                    ...
                    raise AssertionError("network observation ordinary destroy left runtime residue")
                sessions = list(capture_root.glob("live-span-*"))
        ...
        cleanup()
    print("GraphX network observation live capture/fault lifecycle passed three times")
    return 0
