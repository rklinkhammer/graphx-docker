...
Preserved intent and proposed acceptance: Two radios, processors and separate sinks; all port maxima remain one; same shared types; control/sample feedback declared, readiness does not cycle.

Current source evidence: [source](../../../../examples/sdr-node/common/processor.py). Source inspection establishes the current contract, not that v3 generation works. All expected artifacts have status `static-design`; proposed execution has not run. Native platform packaging and generic bindings are proposed in every set.
...
- `planned-portable`: verify both radio control paths and result paths resolve independently; N12 rejects combining producers at a single one-input sink.
