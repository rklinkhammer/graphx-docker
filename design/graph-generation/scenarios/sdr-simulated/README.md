...
- `planned-docker`: raw application capture is socket-level evidence. Ethernet SPAN and injected packet loss require separately selected OVS observation acceptance; they are not established by this portable set. Preserve SDR wire bytes, mTLS rejection, source filtering and result calculations.
