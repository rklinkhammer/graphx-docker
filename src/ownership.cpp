...
bool timer_owned(std::uint32_t pid, const std::string& start_time) {
  return pid != 0 && !start_time.empty() && process_start_time(pid) == start_time &&
         process_name(pid) == "graphx-m7-timer";
}
...
int export_owned_network_capture(const GraphConfig& config,
  ...
  const auto lock_path = state_root / (config.id + ".lock");
  FileDescriptor lock(::open(lock_path.c_str(), O_RDONLY | O_NOFOLLOW));
  if (lock.get() < 0 || ::flock(lock.get(), LOCK_SH | LOCK_NB) != 0)
    throw std::runtime_error("cannot lock network capture ownership state");
  const auto state = load_state(state_path);
