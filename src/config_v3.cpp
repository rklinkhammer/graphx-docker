  ...
  for (const auto& connection : config.resolved.at("connections").array()) {
    ...
    if (transport == "tcp") {
      ...
      edge.transport = edge.data_plane == "external"
                           ? TransportSettings(ExternalTransportConfig{tcp})
      ...
      udp.send_buffer_bytes =
          static_cast<std::uint32_t>(settings.at("send_buffer_bytes").integer());
      udp.framing = settings.at("framing").text();
      edge.transport = edge.data_plane == "external"
                           ? TransportSettings(ExternalTransportConfig{udp})
